...
<div x-data="cosmeBotWidget()" x-init="init()"
    ...
    <button type="button"
        ...
        @click="onBubbleClick()"
        class="relative w-16 h-16 rounded-full shadow-card bg-white border border-indigo-100 grid place-items-center hover:shadow-lg transition"
        aria-label="Mở CosmeBot">
        <span class="pulse-ring"></span>
        <svg class="w-9 h-9 bob" viewBox="0 0 64 64" fill="none" aria-hidden="true">
            <circle cx="32" cy="32" r="28" fill="#E0E7FF" stroke="#818CF8" stroke-width="2" />
            <circle cx="24" cy="28" r="4" fill="#1E293B" />
            <circle cx="40" cy="28" r="4" fill="#1E293B" />
            <path d="M22 40c3 3 7 5 10 5s7-2 10-5" stroke="#1E293B" stroke-width="3" stroke-linecap="round" />
            <circle cx="18" cy="34" r="3" fill="#818CF8" opacity=".7" />
            <circle cx="46" cy="34" r="3" fill="#818CF8" opacity=".7" />
        </svg>
    </button>
    ...
    <div x-show="open" x-transition
        ...
        <div class="bg-white border border-indigo-100 rounded-2xl shadow-card overflow-hidden h-full flex flex-col">

            {{-- Header (drag handle) --}}
            <div class="px-4 py-3 bg-indigo-50/60 border-b border-indigo-100 flex items-center gap-2 cursor-grab active:cursor-grabbing"
                @pointerdown="startDrag($event,'header')" style="touch-action:none">
                <div class="w-7 h-7 rounded-full grid place-items-center bg-white border border-indigo-100">
                ...
                <div class="min-w-0">
                    ...
                    <div class="text-[11px] text-slate-600">Tư vấn tận tâm ✨</div>
                </div>
                <button class="ml-auto text-slate-400 hover:text-slate-700" @click="open=false" aria-label="Đóng">
            ...
            <div id="botScroll" class="flex-1 overflow-y-auto px-3 py-3 space-y-2 text-sm bg-white">
                ...
                ? 'inline-block px-3 py-2 rounded-2xl bg-indigo-600 text-white'
                : 'inline-block px-3 py-2 rounded-2xl bg-indigo-50/70 border border-indigo-100'">
                        ...
                        <div class="mt-1 flex gap-2 flex-wrap" x-show="m.suggestions && m.suggestions.length">
                            <template x-for="s in m.suggestions" :key="s">
                                <button class="px-2 py-1 text-xs rounded-full border border-indigo-200 hover:bg-indigo-50 text-indigo-700"
                        ...
                        <div class="mt-2 grid grid-cols-2 gap-2" x-show="m.products && m.products.length">
                            <template x-for="p in m.products" :key="p.url">
                                <a :href="p.url" class="border border-indigo-100 rounded-xl overflow-hidden hover:shadow-card bg-white js-card">
                                    <span class="shine"></span>
                                    <img :src="p.img" class="w-full h-24 object-contain bg-white" alt="">
                                    <div class="p-2">
                                        <div class="line-clamp-2 text-[12px] font-medium text-slate-800" x-text="p.name"></div>
                                        <div class="mt-1">
                                            <span class="text-[13px] font-bold text-indigo-600" x-text="p.price"></span>
                                            <span class="text-[11px] text-slate-400 line-through" x-show="p.compare" x-text="p.compare"></span>
                                            <span class="ml-1 text-[11px] text-white bg-indigo-600 px-1 rounded" x-show="p.discount" x-text="'-'+p.discount+'%'"></span>
                                        </div>
                                    </div>
                                </a>
                            </template>
                ...
                <div x-show="typing" class="text-left">
                    <div class="inline-block px-3 py-2 rounded-2xl bg-indigo-50/70 border border-indigo-100">
                        <span class="inline-block w-2 h-2 rounded-full bg-indigo-400 animate-pulse mr-1"></span>
                        <span class="inline-block w-2 h-2 rounded-full bg-indigo-400 animate-pulse mr-1"></span>
            ...
            {{-- Input --}}
            <form @submit.prevent="send()" class="p-3 border-t border-indigo-100 flex items-center gap-2 bg-white">
                @csrf
                <input type="text" x-model="input" placeholder="Hỏi CosmeBot điều gì cũng được nè…"
                    class="flex-1 px-3 py-2 rounded-full border border-indigo-200 outline-none focus:ring-2 focus:ring-indigo-300">
                <button class="w-10 h-10 rounded-full bg-indigo-600 text-white grid place-items-center hover:bg-indigo-700 transition">
...
<style>
    ...
    .pulse-ring::after {
        ...
        position: absolute;
        inset: -6px;
        border: 2px solid rgba(79, 70, 229, .25); /* Indigo color */
        border-radius: 9999px;
    ...
    .js-card:hover .shine { opacity: 1; }
</style>

<script>
    // Giữ nguyên toàn bộ logic JS như cũ
    (function() {
    ...
    function cosmeBotWidget() {
        return {
            ...
            updateSide() { this.panelSide = (this.x > window.innerWidth / 2) ? 'right' : 'left'; },
            updatePlacement() {
            ...
            push(role, html, extra = {}) {
                this.messages.push({ id: this.id++, role, html, ...extra });
                this.scrollBottom();
            },
