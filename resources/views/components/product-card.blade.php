...
<div {{ $attributes->merge(['class' => 'relative wave-card js-card']) }}>
    <div class="relative rounded-3xl border border-sky-400/90 bg-gradient-to-br from-sky-600 via-cyan-400 to-sky-200 shadow-2xl shadow-sky-500/50 hover:shadow-2xl hover:shadow-sky-600/40 transition-all duration-300 hover:-translate-y-1 ring-1 ring-sky-300/70">
        ...
        <div x-data="{ faved: {{ in_array($product->id, (array)session('wishlist', [])) ? 'true' : 'false' }} }"
            ...
            <button type="button"
                ...
                class="relative grid place-items-center w-10 h-10 rounded-full bg-white/95 backdrop-blur
                     ring-1 ring-fuchsia-200/70 shadow hover:ring-fuchsia-300 hover:shadow-md
                     focus:outline-none focus:ring-2 focus:ring-fuchsia-400 transition">
                <i :class="faved ? 'fa-solid fa-heart text-sky-600 text-[18px]' : 'fa-regular fa-heart text-gray-700 text-[18px]'"></i>
            </button>
        </div>
        ...
        <div class="absolute top-3 right-3 z-10">
            <span class="inline-block bg-white text-sky-700 text-xs font-bold px-2 py-1 rounded-md shadow-lg shadow-sky-200/60">
                -{{ $discount }}%
            </span>
        </div>
        @endif
        ...
        <a href="{{ $href }}" class="block p-3 group">
            {{-- Khung ảnh --}}
            <div class="relative rounded-xl bg-white h-44 md:h-48 grid place-items-center overflow-hidden shadow-inner shadow-pink-200/40">
            ...
            <div class="mt-3 min-h-[44px]">
                <div class="line-clamp-2 text-[15px] leading-5 font-semibold text-white group-hover:text-slate-100 transition-colors">
                    {{ $product->name }}
                </div>
            </div>

            {{-- Đã bán + Sao đánh giá --}}
            <div class="mt-1 text-[12px] text-slate-100 flex items-center gap-2">
                <span>Đã bán <span class="font-semibold text-white">{{ number_format($product->sold_count ?? 0) }}</span></span>

                <span class="inline-block w-px h-3 bg-gray-200"></span>
                ...
                <span class="flex items-center gap-0.5">
                    ...
                    @for($i = 1; $i <= 5; $i++)
                        ...
                        @else
                        <i class="fa-regular fa-star text-slate-200 text-[11px]"></i>
                        @endif
                        @endfor
                        <span class="ml-1 text-[11px] text-slate-200">{{ number_format((float)$avg, 1) }}</span>
                        @else
            ...
            <div class="mt-2">
                ...
                <div class="flex items-baseline gap-2">
                    <div class="text-[18px] font-extrabold text-white">
                        {{ number_format($price) }}₫
                    </div>
                    <div class="text-xs text-slate-200/90 line-through">
                ...
                @else
                <div class="text-sm text-slate-200">Liên hệ</div>
                @endif
            </div>
        </a>
    </div>
