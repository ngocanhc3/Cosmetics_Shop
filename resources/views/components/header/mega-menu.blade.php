...
<ul class="flex items-center gap-6 whitespace-nowrap">
    ...
    <li class="relative group pb-2">
        <a href="{{ route('category.show', $cat->slug) }}"
            class="inline-flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-pink-50">
            {{ $cat->name }}
        ...
        @if(!empty($cat->children) && count($cat->children))
        <div
            class="absolute left-0 top-full w-[min(960px,calc(100vw-2rem))] bg-white border border-pink-100
            ...
            <div class="grid grid-cols-3 gap-6">
                ...
                <div>
                    <a href="{{ route('category.show', $child->slug) }}"
                        class="font-medium hover:text-pink-600">{{ $child->name }}</a>
