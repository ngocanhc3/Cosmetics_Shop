...
'huy' => 'bg-pink-50 text-pink-700 border-pink-200',
];
$payMap = [
'paid' => 'bg-emerald-50 text-emerald-700 border-emerald-200',
'pending' => 'bg-amber-50 text-amber-700 border-amber-200',
'failed' => 'bg-pink-50 text-pink-700 border-pink-200',
'refunded' => 'bg-sky-50 text-sky-700 border-sky-200',
];
$map = $type==='status' ? $statusMap : $payMap;
return 'inline-flex items-center rounded-full border px-2 py-0.5 text-xs '.($map[$val] ?? 'bg-pink-50/60 text-ink/70 border-pink-200');
};
...
<div class="max-w-7xl mx-auto px-4 py-6">
    ...
    <div class="flex items-start justify-between gap-4">
        ...
        <a href="{{ route('shop.index') }}"
            class="hidden md:inline-flex items-center rounded-xl bg-pink-600 text-white px-4 py-2 text-sm hover:bg-pink-500">
            Tiếp tục mua sắm
        </a>
    </div>
    ...
    <div class="mt-4 flex flex-wrap gap-2">
        ...
        $chipNormal = 'bg-white text-ink border-pink-200 hover:bg-pink-50';
        @endphp
    ...
    <form method="get" class="grid grid-cols-1 md:grid-cols-6 gap-3 mt-4 mb-5">
        <div class="col-span-1 md:col-span-2 relative">
            ...
            <input name="q" value="{{ $filters['q'] }}" placeholder="Tìm mã đơn (VD: CH-...)"
                class="w-full rounded-xl border border-pink-200 bg-white pl-9 pr-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-pink-300" />
        </div>

        <select name="status" class="col-span-1 md:col-span-1 w-full rounded-xl border border-pink-200 bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-pink-300">
        ...
        <select name="payment" class="col-span-1 md:col-span-1 w-full rounded-xl border border-pink-200 bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-pink-300">
        ...
        <input type="date" name="from" value="{{ $filters['from'] }}"
            class="col-span-1 md:col-span-1 w-full rounded-xl border border-pink-200 bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-pink-300" />
        <input type="date" name="to" value="{{ $filters['to'] }}"
            class="col-span-1 md:col-span-1 w-full rounded-xl border border-pink-200 bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-pink-300" />

        <div class="col-span-1 md:col-span-6 flex gap-2">
            <button class="inline-flex items-center rounded-xl bg-pink-600 text-white px-4 py-2.5 text-sm hover:bg-pink-500">Lọc</button>
            <a href="{{ route('account.orders.index') }}" class="inline-flex items-center rounded-xl border border-pink-200 px-4 py-2.5 text-sm hover:bg-pink-50">Xoá lọc</a>
        </div>
    </form>

    {{-- Desktop table --}}
    <div class="hidden md:block bg-white border border-pink-100 rounded-2xl shadow-sm overflow-hidden">
            ...
            <tbody class="divide-y divide-pink-100">
                ...
                <tr class="hover:bg-pink-50/40 transition">
                    ...
                    <td class="px-6 py-3 text-right space-x-2">
                        ...
                        <a href="{{ route('payment.vietqr.show', $o->id) }}"
                            class="inline-flex items-center rounded-xl bg-pink-600 text-white px-3 py-1.5 text-sm hover:bg-pink-500">
                            Thanh toán
                        </a>
                        @endif
                        <a href="{{ route('account.orders.show', $o->id) }}"
                            class="inline-flex items-center rounded-xl border border-pink-200 px-3 py-1.5 text-sm hover:bg-pink-50">
                            Chi tiết
                        </a>
                    </td>
                </tr>
                ...
                <tr>
                    <td colspan="6" class="px-6 py-10 text-center">
                        <div class="mx-auto w-12 h-12 rounded-full bg-pink-100 flex items-center justify-center mb-3">
                        ...
                        <a href="{{ route('shop.index') }}"
                            class="mt-3 inline-flex items-center rounded-xl bg-pink-600 text-white px-4 py-2 text-sm hover:bg-pink-500">
                            Tiếp tục mua sắm
                        </a>
                    </td>
                </tr>
        ...
        @if($orders->hasPages())
        <div class="px-6 py-3 border-t border-pink-100">
            {{ $orders->links() }}
        </div>
        @endif
    </div>
        ...
        <div class="bg-white border border-pink-100 rounded-2xl shadow-sm p-4">
            ...
            <div class="mt-3 flex items-center justify-between">
                ...
                <div class="space-x-2">
                    @if($o->payment_status!=='paid' && \Illuminate\Support\Facades\Route::has('payment.vietqr.show') && ($o->payment_method ?? '')==='VIETQR')
                    <a href="{{ route('payment.vietqr.show', $o->id) }}"
                        class="inline-flex items-center rounded-xl bg-pink-600 text-white px-3 py-1.5 text-xs hover:bg-pink-500">Thanh toán</a>
                    @endif
                    <a href="{{ route('account.orders.show', $o->id) }}"
                        class="inline-flex items-center rounded-xl border border-pink-200 px-3 py-1.5 text-xs hover:bg-pink-50">Chi tiết</a>
                </div>
            </div>
        </div>
        @empty
        <div class="bg-white border border-pink-100 rounded-2xl shadow-sm p-6 text-center">
            <div class="mx-auto w-12 h-12 rounded-full bg-pink-100 flex items-center justify-center mb-3">
                <svg class="w-6 h-6 text-pink-600" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                        d="M3 3h2l.4 2M7 13h10l3-7H6.4M7 13l-1.2 5.2A1 1 0 007 20h10" />
