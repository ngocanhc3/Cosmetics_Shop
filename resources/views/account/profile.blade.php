...
<div class="max-w-4xl mx-auto px-4 py-6">
    ...
    <form method="POST" action="{{ route('account.profile.update') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
        ...
        <div class="bg-white border border-pink-100 rounded-xl shadow-card p-5">
            <div class="grid grid-cols-1 md:grid-cols-[160px,1fr] gap-6 items-start">
                {{-- Avatar --}}
                <div x-data="{preview: '{{ $avatar ?? '' }}' }" class="space-y-3">
                    <div class="w-36 h-36 rounded-full bg-pink-50 border border-pink-100 overflow-hidden flex items-center justify-center">
                    ...
                    <label class="inline-flex items-center px-3 py-2 rounded-md border text-sm hover:bg-pink-50 cursor-pointer">
                    ...
                    <label class="inline-flex items-center gap-2 text-sm">
                        <input type="checkbox" name="remove_avatar" value="1" class="rounded border-pink-300">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $user->name) }}"
                            class="w-full rounded-md border border-pink-200 bg-white px-3 py-2 outline-none focus:ring-2 focus:ring-brand-400">
                    </div>
            ...
            <div class="mt-6 flex items-center gap-3">
                ...
                <a href="{{ route('account.dashboard') }}" class="inline-flex items-center rounded-md border px-4 py-2 text-sm hover:bg-pink-50">
                    Quay lại tổng quan
                </a>
            </div>
        </div>
