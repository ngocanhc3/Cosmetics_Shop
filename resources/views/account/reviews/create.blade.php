...
<div class="bg-pink-50/60 border border-pink-200 rounded-2xl p-4 md:p-5 flex gap-4 items-start shadow-card">
    <img src="{{ $thumb ?: 'https://placehold.co/96' }}"
         class="w-20 h-20 md:w-24 md:h-24 object-contain bg-white border border-pink-100 rounded-xl shadow-sm"
         alt="Ảnh sản phẩm">
    <div class="flex-1">
        <p class="text-xs md:text-sm text-ink/60">
            Mã đơn: <b>{{ $order->code }}</b>
        </p>
        <h1 class="text-lg md:text-xl font-semibold mt-0.5">{{ $product->name }}</h1>
        @if($item->variant_name ?? false)
            <span class="mt-1 inline-flex rounded-full border px-2 py-0.5 text-xs text-ink/70">
                {{ $item->variant_name }}
            </span>
        @endif
    </div>
...
<form id="review-form"
    ...
    {{-- Tiêu đề --}}
    <div>
        <label for="title" class="text-sm font-medium">Tiêu đề (tuỳ chọn)</label>
    ...
    {{-- Nội dung --}}
    <div>
        <label for="review-content" class="text-sm font-medium">Nội dung</label>
