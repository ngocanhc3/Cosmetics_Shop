...
<div class="space-y-4">
    @forelse($reviews as $rv)
        <div class="p-4 border border-pink-100 rounded-xl bg-white shadow-sm">
            {{-- Header: tên + trạng thái + sao --}}
            <div class="flex items-center justify-between gap-2">
                <div class="font-medium flex items-center gap-1.5">
                    {{ optional($rv->user)->name ?: 'Ẩn danh' }}
                    @if($rv->verified_purchase)
                        <span class="px-2 py-0.5 text-emerald-700 bg-emerald-50 border border-emerald-200 rounded text-xs">
                            Đã mua
                        </span>
                    @endif
                    @if(!$rv->is_approved && auth()->id() === $rv->user_id)
                        <span class="px-2 py-0.5 text-amber-700 bg-amber-50 border border-amber-200 rounded text-xs">
                            Chờ duyệt
                        </span>
                    @endif
                </div>
