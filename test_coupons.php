...
$q = DB::table('user_coupons as uc')
    ->join('coupons as c', function($j) use ($hasCouponId,$hasCouponCode){
        if($hasCouponId){
            ...
            if($hasCouponCode){ $j->orOn('c.code','=','uc.coupon_code'); }
        } elseif($hasCouponCode){ $j->on('c.code','=','uc.coupon_code'); }
    })
    ->where('uc.user_id',$uid);
if($hasTimes){
    $q->select(array_merge($couponColumns,['uc.times']))->where('uc.times','>',0);
} else {
    $q->select(array_merge($couponColumns,[DB::raw('COUNT(*) as times')]))->groupBy('c.id','c.code','c.name');
}
